...
int setData(const char *token, const char *topic, int val) {
    ...
    int httpResponseCode =
        http_client.POST((uint8_t *)ouput.c_str(), ouput.length());
...
/*
Get a set of data from the specified data list and store it in the specified
array 从指定的数据列表中获取一组数据并存储在指定数组里 list:  list name.
列表名称 offset: the offset relative to the first position of the data list.
相对于数据列表首位的偏移 count: the number of read data.  读取数据个数
*/
int *getData(const char *token, const char *list, int *Array, int offset,
             int count) {
    String req = (String)host + '/' + token + '/' + list +
